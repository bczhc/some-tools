...
public class PaintView extends BaseView {
    ...
    public PathBeanItem deleteLastPathBeanItem(ArrayList<PathBeanItem> list, boolean isImportingMode) {
        int indexToRemove = -1;
        for (int i = list.size() - 1; i >= 0; i--) {
            PathBeanItem item = list.get(i);
            if (item.isImportingMode() == isImportingMode) {
                indexToRemove = i;
                break;
            }
        }

// 删除最后一个符合 isImportingMode 的 PathBeanItem
        PathBeanItem lastPathBeanItemOfIsImportingMode = list.get(indexToRemove);
        list.remove(indexToRemove);
        return lastPathBeanItemOfIsImportingMode;

    }

    public int getCountOfPathBeanItemsWithImportingMode(ArrayList<PathBeanItem> list, boolean importingMode) {
        int count = 0;
        for (PathBeanItem item : list) {
            if (item.isImportingMode() == importingMode) {
                count++;
            }
        }
        return count;
    ...
    public void removeAllPathBeanItems(ArrayList<PathBeanItem> list, boolean importingMode) {
        Iterator<PathBeanItem> iterator = list.iterator();
        while (iterator.hasNext()) {
            PathBeanItem item = iterator.next();
            if (item.isImportingMode() == importingMode) {
                iterator.remove();
            }
        }
    ...
    public boolean undo(boolean isImportingMode) {
        ...
        if (canUndo() || (isImportingMode && getCountOfPathBeanItemsWithImportingMode(undoListRef, true) > 0)) {
            ...
            clearPaint();//清除之前绘制内容
            PathBeanItem lastPbi;
            if (isImportingMode) {
                if (getCountOfPathBeanItemsWithImportingMode(undoListRef, true) > 0) {
                    lastPbi = deleteLastPathBeanItem(undoListRef, true);//将最后一个移除
                } else {
                    lastPbi = deleteLastPathBeanItem(undoListRef, false);//将最后一个移除
                    lastPbi.setImportingMode(true);
                }
            } else {
                lastPbi = deleteLastPathBeanItem(undoListRef, false);//将最后一个移除
            }
            redoListRef.add(lastPbi);//加入 恢复操作
            //遍历，将Path重新绘制到 headCanvas
            if (showDrawing) {
                for (PathBeanItem pbi : undoListRef) {
                    mCanvas.drawPath(pbi.getBody().path, pbi.getBody().paint);
                }
                postInvalidate();
            }
            return true;
     ...
     * 恢复操作
     */
    public boolean redo(boolean isImportingMode) {
        if (canRedo() || (isImportingMode && getCountOfPathBeanItemsWithImportingMode(redoListRef, true) > 0)) {
            layerPathSaverRef.redo();
            PathBeanItem pathBeanItem;
            if (isImportingMode) {
                pathBeanItem = deleteLastPathBeanItem(redoListRef, true);
                pathBeanItem.setImportingMode(false);
                removeAllPathBeanItems(redoListRef, false);
                // 点击下一步时清空恢复列表
            } else {
                pathBeanItem = deleteLastPathBeanItem(redoListRef, false);
            }
            mCanvas.drawPath(pathBeanItem.getBody().path, pathBeanItem.getBody().paint);
            undoListRef.add(pathBeanItem);
            if (showDrawing) {
                postInvalidate();
            }
            return true;
    ...
    private Bitmap drawPathsToBitmap(@NotNull List<PathBeanItem> pathBeanItems, @Nullable Matrix matrix, int width, int height, FloatCallback progressCallback) {
        final Bitmap bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);

        Canvas canvas = new Canvas(bitmap);
        canvas.setMatrix(matrix);

        final int size = pathBeanItems.size();

        float count = 0F;
        for (PathBeanItem pathBeanItem : pathBeanItems) {
    ...
    public void exportImg(File f, int width, int height, ImageExportProgressCallback progressCallback) throws IOException {
        ...
        for (int i = layerArray.size() - 1; i >= 0; i--) {
            ...
            final ArrayList<PathBeanItem> undoList = layer.undoList;
...
//        JNI.Bitmap.compressToPng(resultBitmap, f.getPath());
    ...
    public boolean canUndo() {
        return getCountOfPathBeanItemsWithImportingMode(undoListRef, false) != 0;
    }
    ...
    public boolean canRedo() {
        return getCountOfPathBeanItemsWithImportingMode(redoListRef, false) != 0;
    }
    ...
    public void importPathVer1_0(@NotNull File f, @Nullable PathImportCallback progressCallback) throws IOException {
        ...
        boolean stepImportMark = false; // 已经因为两个up判断，指针移动过了，直接短路导入指针移动逻辑
        int lastMotionAction = -1;
        // 完成现有对步进导入的处理
        while (stepImportMark || (pathImportingLastStepCount > 0 && !pathRollBackAndFinishOneStep) || is.read(bytes) != -1) {
            Log.i("fdb-test", "短路导入判断:" + (stepImportMark || (pathImportingLastStepCount > 0 && !pathRollBackAndFinishOneStep)));
            stepImportMark = false;
            ...
            if (pathImportingLastStepCount == 0 || (pathRollback && pathImportingOneStep)) {
                ...
                read += 26L;
                if (lastMotionAction == 1 && JNI.FloatingBoard.byteArrayToInt(bytes_4, 0) == 1) {
                    is.read(bytes);
                    stepImportMark = true;
                    Log.i("feb-test", "两个up，指针后移");
                } else {
                    switch (bytes[25]) {
                        case 1:
                    ...
                    if (bytes[25] == 1 || bytes[25] == 2) {
                        lastMotionAction = -1;
                    } else {
                        lastMotionAction = JNI.FloatingBoard.byteArrayToInt(bytes_4, 0);
                    }
                }
            }
    ...
    public void importPathVer2_0(@NotNull File f, @Nullable PathImportCallback progressCallback) throws IOException {
        ...
        while (stepImportMark || (pathImportingLastStepCount > 0 && !pathRollBackAndFinishOneStep) || is.read(bytes) != -1) {
            ...
            if (pathImportingLastStepCount == 0 || (pathRollback && pathImportingOneStep)) {
                ...
                if (lastActionMark == 3 && x != -1 && y != -1 && p1 == 3) {
                    is.read(bytes);
                    stepImportMark = true;
                    Log.i("fdb-test", "两个up，指针后移");
                } else {
                    switch (p1) {
                        case 4:
                            undo();
    ...
    public void importPathVer2_1(@NotNull File f, @Nullable PathImportCallback progressCallback) throws IOException {
        ...
        int lastActionMark = -1;
        // 完成现有对步进导入的处理
        while (stepImportMark || (pathImportingLastStepCount > 0 && !pathRollBackAndFinishOneStep) || reader.read(buffer) == 9) {
            Log.i("fdb-test", "短路导入判断:" + (stepImportMark || (pathImportingLastStepCount > 0 && !pathRollBackAndFinishOneStep)));
            stepImportMark = false;
            // noinspection StatementWithEmptyBody
            while (pathImportPaused && !isImportingTerminated && !pathImportingOneStep) {
            ...
            if (pathImportingLastStepCount == 0 || (pathRollback && pathImportingOneStep)) {
                // 仅在无因点击“上一步”而撤回的步骤或第一次点击“上一步”（或“上一步”撤回的步骤导入完成后）把剩余未完成的一步导入完 时，进行正常的导入操作
                if (lastActionMark == 0xB2 && buffer[0] == 0xB2) {
                    reader.read(buffer);
                    stepImportMark = true;
                    Log.i("fdb-test", "两个up，指针后移");
                } else {
...
//        don'tDrawWhileImporting = speedDelayMillis == 0;
        ...
        int c = 0;
        while (stepImportMark || (pathImportingLastStepCount > 0 && !pathRollBackAndFinishOneStep) || cursor.step()) {
            Log.i("fdb-test", "短路导入判断:" + (stepImportMark || (pathImportingLastStepCount > 0 && !pathRollBackAndFinishOneStep)));
            stepImportMark = false;
            // noinspection StatementWithEmptyBody
            while (pathImportPaused && !isImportingTerminated && !pathImportingOneStep) {
            ...
            if (pathImportingLastStepCount == 0 || (pathRollback && pathImportingOneStep)) {
                ...
                if ((lastActionMark == 0x04 || lastActionMark == 0x14) && (mark == 0x04 || mark == 0x14)) {
                    cursor.step();
                    stepImportMark = true;
                    Log.i("fdb-test", "两个up，指针后移");
                } else {
    ...
    private void importLayerPath4_0(
        ...
        while (stepImportMark || (pathImportingLastStepCount > 0 && !pathRollBackAndFinishOneStep) || cursor.step()) {
            ...
            if (pathImportingLastStepCount == 0 || (pathRollback && pathImportingOneStep)) {
                // 仅在无因点击“上一步”而撤回的步骤或第一次点击“上一步”（或“上一步”撤回的步骤导入完成后）把剩余未完成的一步导入完 时，进行正常的导入操作
                int mark = cursor.getInt(0);
                if ((lastActionMark == 0x13 || lastActionMark == 0x03) && (mark == 0x13 || mark == 0x03)) {
                    cursor.step();
                    stepImportMark = true;
                    Log.i("fdb-test", "两个up，指针后移");
                } else {
        ...
        switch (motionAction) {
            ...
            case MotionEvent.ACTION_UP:
                if (mPath != null) {
                    ...
                    // for undoing
                    undoListRef.add(new PathBeanItem(pb));
                    redoListRef.clear();
                    mPath.reset();
                    mPath = null;
                }
    ...
    public class PathBeanItem {
        public PathBean body;
        public boolean isImportingMode;

        public PathBeanItem(PathBean body) {
            this(body, false);
        }

        public PathBeanItem(PathBean body, boolean isImportingMode) {
            this.body = body;
            this.isImportingMode = isImportingMode;
        }

        public PathBean getBody() {
            return body;
        }

        public void setBody(PathBean newBody) {
            body = newBody;
        }

        public boolean isImportingMode() {
            return isImportingMode;
        }

        public void setImportingMode(boolean newIsImportingMode) {
            isImportingMode = newIsImportingMode;
        }
    }
